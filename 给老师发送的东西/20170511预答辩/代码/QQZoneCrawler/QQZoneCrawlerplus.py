...
class seleniumTest(unittest.TestCase):
    user = '2844577946'  # 测试用QQ号
    pw = 'yzx19941112'  # 测试用QQ密码
...
#         driver.get("https://user.qzone.qq.com/773885456/311")
        ...
        fout.write("<td>设备</td>")
        fout.write("</tr>")
#         fout1 =open('source.html','w',encoding='utf-8')
#         fout2 =open('source2.html','w',encoding='utf-8')
        while True:
            # 下拉滚动条，使浏览器加载出动态加载的内容，可能像这样要拉很多次，中间要适当的延时（跟网速也有关系）。
            # 如果说说内容都很长，就增大下拉的长度。
            print('正在抓取第%s个页面' % count)
...
#             fout2.write(driver.page_source)
            driver.switch_to.frame("app_canvas_frame")
#             fout1.write(driver.page_source)
            
            soup = BeautifulSoup(driver.page_source, 'xml')
            contents = soup.find_all('pre', {'class': 'content'})  # 内容
            times = soup.find_all('a', {'class': 'c_tx c_tx3 goDetail'})  # 发表时间
            devices =  soup.find_all('span', {'class': 'custom-tail'})
#             likes =soup.find_all('a', {'class': 'qz_like_btn c_tx mr8'})
#             print(contents)
#             print(devices)
#             print(likes)
#             break;
            for content, _time,_device in zip(contents, times,devices):  # 这里_time的下划线是为了与time模块区分开
                fout.write("<tr>")
                fout.write("<td>%s</td>" % _time)
                fout.write("<td>%s</td>" % content)
                fout.write("<td>%s</td>" % _device)
                fout.write("</tr>")
            count+=1;
